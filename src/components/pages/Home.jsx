...
function Home() {
  const [popularGame, setPopularGame] = useState('Fornite');
    ...
    const intervalId = setInterval(async () => {
      try {
        ...
        if (response.ok) {
          ...
          setPopularGame(data.popular); 
        } else {
          console.error('Error al obtener el juego popular:', response.status);
        }
      } catch (error) {
        console.error('Error en la solicitud de short polling:', error);
      }
    }, 5000); 
